...
class Weather:
    def __init__(self, latitude, longitude, api_id):
        ...
        self.data = requests.get(
            f"https://api.openweathermap.org/data/2.5/onecall?lat={self.latitude}&lon={self.longitude}&lang=de&appid={self.api_key}").json()
    ...
    def weather_description(self, id):
        ...
        if id // 100 != 8:
            ...
            if id == 801:
                icon = "25_clouds"
                weather_detail = "Teilweise wolkig"
            elif id == 802:
                icon = "50_clouds"
